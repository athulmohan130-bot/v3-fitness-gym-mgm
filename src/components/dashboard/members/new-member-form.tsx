...
import { Camera, CalendarIcon, Loader2, ArrowLeft, ArrowRight, User as UserIcon, RotateCcw } from "lucide-react";
...
import { collection, serverTimestamp, doc, writeBatch, runTransaction, FieldValue, increment, setDoc } from "firebase/firestore";
...
const formSchema = z.object({
  ...
  emergencyContactRelation: z.string({ required_error: "Please select a relationship." }),
});

const STEPS = [
  { id: 'personal', title: 'Personal Information', fields: ['name', 'email', 'phone', 'gender', 'dateOfBirth', 'address'] },
  { id: 'health', title: 'Health & Fitness', fields: ['heightCm', 'weightKg', 'fitnessGoal', 'medicalConditions'] },
  { id: 'picture', title: 'Profile Picture', fields: ['profilePicture'] },
  { id: 'membership', title: 'Membership & Role', fields: ['membershipPlanId', 'joinDate', 'role'] },
  { id: 'emergency', title: 'Emergency Contact', fields: ['emergencyContactName', 'emergencyContactPhone', 'emergencyContactRelation'] },
];
...
export function NewMemberForm({ plans }: NewMemberFormProps) {
  ...
  const capturedImage = useWatch({ control: form.control, name: 'profilePicture' });
  ...
  async function processForm(values: z.infer<typeof formSchema>) {
    if (!firestore || !adminUser) {
        toast({ variant: 'destructive', title: 'Error', description: 'You must be logged in to create a member.' });
        return;
    }
    ...
    try {
        ...
        if (values.profilePicture) {
            const storageRef = ref(storage, `profile_images/${newUserRef.id}_${new Date().getTime()}.png`);
        ...
        const membershipEnd = add(membershipStart, { days: selectedPlan.durationInDays });
        ...
        toast({ title: "Member Profile Created!", description: `${values.name}'s profile has been created. Now, create their login credentials in Firebase Authentication.` });
        router.push("/dashboard/members");

    } catch (error) {
        console.error("Error creating member:", error);
        toast({ variant: "destructive", title: "Uh oh!", description: "Could not create member profile. Make sure the stats documents are initialized in Firestore." });
    } finally {
        setIsLoading(false);
    }
  }
    ...
    <div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(processForm)} className="space-y-8">
          ...
          <div className={cn(currentStep !== 1 && "hidden")}>
            <Card>
              <CardHeader><CardTitle>Health & Fitness</CardTitle></CardHeader>
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField control={form.control} name="medicalConditions" render={({ field }) => (
                  <FormItem className="md:col-span-2">
                    ...
                    <FormControl><Textarea placeholder="List any relevant medical history..." {...field} /></FormControl>
          ...
          <div className={cn(currentStep !== 2 && "hidden")}>
            <Card>
              <CardHeader>
                ...
                <CardDescription>Capture a photo of the new member.</CardDescription>
              </CardHeader>
              <CardContent className="flex flex-col items-center gap-4">
                <div className="w-full max-w-sm aspect-video rounded-md border bg-muted flex items-center justify-center overflow-hidden">
                  {capturedImage ? (
                    <Image src={capturedImage} alt="Captured photo" width={400} height={225} className="object-cover" />
                  ) : stream ? (
                ...
                <video ref={videoRef} className={cn("hidden", { "block w-full max-w-sm aspect-video rounded-md": stream && !capturedImage })} autoPlay muted playsInline />
                ...
                <div className="flex gap-2">
                  ...
                  {capturedImage && (
                    <Button type="button" variant="outline" onClick={recapturePhoto}>
          ...
          <div className={cn(currentStep !== 3 && "hidden")}>
            <Card>
              <CardHeader><CardTitle>Membership & Role</CardTitle></CardHeader>
              ...
              <CardContent className="space-y-6">
                ...
                <FormField control={form.control} name="emergencyContactPhone" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="+91 876 543 2109" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
      ...
      <div className="mt-8 pt-5 flex justify-between">
        ...
        {currentStep < STEPS.length - 1 ? (
          ...
          <Button type="button" onClick={form.handleSubmit(processForm)} disabled={isLoading}>
