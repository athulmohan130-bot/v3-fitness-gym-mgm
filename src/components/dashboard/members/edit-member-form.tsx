...
import { Camera, CalendarIcon, Loader2, User as UserIcon, RotateCcw } from "lucide-react";
...
export function EditMemberForm({ plans, member }: EditMemberFormProps) {
    ...
    const revertPhoto = () => {
        form.setValue('profilePicture', initialProfilePicture, { shouldValidate: true });
        if(stream) {
           stream.getTracks().forEach(track => track.stop());
           setStream(null);
        }
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        try {
            ...
            toast({ variant: 'destructive', title: "Update Failed", description: "An error occurred while updating the member." });
        } finally {
            setIsLoading(false);
        }
    }
                            ...
                            <CardContent className="flex flex-col items-center gap-4">
                                <div className="w-full max-w-sm aspect-video rounded-md border bg-muted flex items-center justify-center overflow-hidden">
                                     {stream ? (
                                        ...
                                        <Image src={capturedImage} alt="Captured photo" width={400} height={225} className="object-cover" data-ai-hint="person gym" />
                                    ) : (
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {capturedImage && capturedImage !== initialProfilePicture && !stream && (
                                        <Button type="button" variant="outline" onClick={revertPhoto}>
